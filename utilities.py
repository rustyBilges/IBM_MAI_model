...
def write_spatial_state(ecosystem, iteration, output_dir='../output', save_habitats=False):
    ...
    if save_habitats:
    	    filename_habitat = output_dir+'/output_habitat_'+ str(iteration) +'.csv'
	    Z = []
	    for i in range(ROWS):
		    row = []
		    for j in range(COLUMNS):
			    if ecosystem.world[i][j].habitat == None:
			    print 'This cell does not have habitat', i, j 
			    row.append(ecosystem.world[i][j].habitat)
		    Z.append(numpy.array(row))
		
	    z = numpy.array(Z)
	    numpy.savetxt(filename_habitat, z, delimiter=',')
